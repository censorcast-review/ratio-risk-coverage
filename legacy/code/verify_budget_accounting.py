...
cache = ROOT / 'results/objectives/cache_design/shadow_aligned.npz'
pred_file = ROOT / 'results/objectives/design_predictions_s20260906.npz'
...
for name, score in [
    ...
    report = ROOT / f'results/conditional_heads/report_{"direct_excess" if name == "direct" else "relative_error_f"}_pooled_s20260906.json'
...
old = json.loads((ROOT / 'results/conditional_heads/SELECTION_MECHANISM.json').read_text())['acceptance_overlap']
rows = {}
...
dump(ROOT / 'results/geometry_and_sensitivity/BUDGET_ACCOUNTING.json', result)
