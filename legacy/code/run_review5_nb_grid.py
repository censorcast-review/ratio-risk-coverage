...
def prepare(data,out,protocol):
    ...
    for key,days in [('validation',protocol['validation_days'])]+list(protocol['blocks'].items()):
        ...
        if key in BLOCKS:
            old=load_npz(ROOT/f'results/objectives/cache_design/{key}_aligned.npz')
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--data',type=Path);p.add_argument('--out',type=Path,default=ROOT/'results/nb_grid');p.add_argument('--freeze-only',action='store_true');a=p.parse_args()
