...
def replay(data,out,protocol):
    ...
    replay_report={}
    for key in BLOCKS:
        retained=load(ROOT/'results/objectives/cache_design'/f'{key}_aligned.npz');idx=retained['target_days']-1314
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--data',type=Path);p.add_argument('--out',type=Path,default=ROOT/'results/error_interventions');p.add_argument('--freeze-only',action='store_true');a=p.parse_args()
