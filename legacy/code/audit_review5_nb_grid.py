...
def run(output):
    ...
    for filename,digest in p['source_hashes'].items():assert sha(__import__('publication_paths').recorded_path('code/'+filename))==digest,filename
