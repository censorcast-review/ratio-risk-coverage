...
def check(out,output):
    ...
    for rel,d in p['source_hashes'].items():assert sha(ROOT/rel)==d,rel
    ...
    published=json.loads((ROOT/'results/review2/objectives/REVIEW2_RESULTS.json').read_text())['head_diagnostics']
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--results',type=Path,default=ROOT/'results/review7_hobbies');ap.add_argument('--output',type=Path,default=ROOT/'tmp/hobbies_verification.json');a=ap.parse_args();check(a.results,a.output)
