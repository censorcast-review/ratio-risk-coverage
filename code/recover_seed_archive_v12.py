...
import sys, json, hashlib, datetime, struct, zlib, io, gc
import numpy as np
...
def main():
    ...
    out=ROOT/'evidence/revision_v12_seeds/archive_recovery'
    ...
    rebuilt=out/'seed_20260914_censored_CALIBRATION.npz'
    np.savez_compressed(rebuilt,**cal)
    with np.load(rebuilt) as z:
        for k,v in cal.items():assert np.array_equal(v,z[k])
    write(out/'COMPLETE.json',dict(utc=now(),fit_calls=0,source_sha256=sha(__file__),protocol_sha256=sha(out/'PROTOCOL.json'),original_sha256=sha(dest/'CALIBRATION.npz'),rebuilt_sha256=sha(rebuilt),exact_retained_array_matches=sorted(retained),rows=len(cal['y']),interpretation='Original incomplete archive preserved; seven arrays replayed from frozen models; no retraining, selection, or result modification. Independent verifier checks all primary thresholds, choices and results.'))
