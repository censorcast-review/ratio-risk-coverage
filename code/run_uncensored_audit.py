...
def main(data,out):
 ...
 for arm in proto['arms']:
  ...
  L=abs(ev['y']-ev['f']);W=ev['y'];units=np.repeat(inv,len(days));total=np.column_stack([np.bincount(units),np.bincount(units,weights=W)]);counts=bootstrap_counts(len(items),SEED,2000);den=counts@total;rows=[]
  for plan in plans:
   menu={};stats={}
...
if __name__=='__main__':
 a=argparse.ArgumentParser();a.add_argument('--data',type=Path,required=True);a.add_argument('--output',type=Path,required=True);x=a.parse_args();main(x.data,x.output)
